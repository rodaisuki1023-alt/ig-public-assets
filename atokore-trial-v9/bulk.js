...
(()=>{
  ...
  const units=['個','本','袋','箱','パック','枚','g','kg','ml','L'];
  let draft=[];
  ...
  function defaultQty(unit){return unit==='g'||unit==='ml'?100:1}
  ...
  function parseLine(line,index){
    ...
    let m=s.match(/^(.*?)(?:\s+|x)(\d+(?:\.\d+)?)\s*(個|本|袋|箱|パック|枚|g|kg|ml|mL|L)\s*$/i);
    if(!m) m=s.match(/^(.*?)(\d+(?:\.\d+)?)\s*(個|本|袋|箱|パック|枚|g|kg|ml|mL|L)\s*$/i);
  ...
  function openBulk(){
    ...
    openM(`<button class="close" onclick="closeM()">閉じる</button>
      <h2>⚡ まとめて追加</h2>
      <div class="bulkAddHint">1行に1商品でまとめて入力できます。数量・単位を書かなくても商品名から自動補完します。</div>
